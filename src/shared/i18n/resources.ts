...
const zhCN = {
  common: {
    app: { title: 'Skill 管理工作台', home: 'Skill 管理工作台首页', author: '暴论哥3.0（公众号同名）', authorName: '暴论哥3.0' },
    action: {
  ...
  workbench: {
    ...
    roots: {
      title: '扫描目录', detail: '原地索引这些目录；添加项目根不会迁移或修改其中的 Skill。', footer: '项目扫描会排除 node_modules、.git、会话缓存和备份。', add: '添加项目根目录', added: '项目根目录已添加，后台扫描已开始', removed: '已移除扫描配置：{{label}}', discovered: '自动发现', lastScan: '{{host}} · {{source}} · 上次扫描 {{date}}', open: '在资源管理器中打开', remove: '移除扫描配置',
    ...
    guide: {
      eyebrow: '首次使用', title: '先认识这张本地 Skill 地图', detail: '工作台会原地索引多个宿主的 Skill，不会把它们搬进自己的目录。', welcomeTitle: '欢迎来到 Skill 管理工作台', welcomeDetail: '先索引，再判断，最后由你确认每一次文件修改。', browse: '开始浏览', scan: '原地建立索引', scanDetail: '自动发现 Codex、Claude、WorkBuddy；项目目录可随时添加，不会迁移现有 Skill。', protect: '先看安全边界', protectDetail: '插件、内置、市场和缓存默认只读；脚本永不执行，删除统一进入工作台回收站。', ai: 'AI 完全按需', aiDetail: '本地规则始终先运行。只有主动点击时才发送已预览的文本，AI 建议不会自动改写文件。', manage: '管理扫描目录', start: '开始使用', note: '首次完整扫描在后台进行，缓存列表会优先显示；顶部状态区可以查看进度。'
    },
    author: {
      description: '认识作者，也可以扫码添加微信、关注公众号或赞赏支持。', footer: '联系与赞赏图片均由作者提供，请使用微信扫码。', done: '知道了', avatarAlt: '暴论哥3.0头像', label: '作者', intro: '你好，我是暴论哥3.0。这是我为本地 Skill 整理、检索和安全管理制作的工作台。更多更新与使用分享，可关注同名公众号。', contact: '联系与关注', scanWechat: '使用微信扫码', wechatAlt: '添加作者微信二维码', addWechat: '添加作者微信', addWechatDetail: '扫码添加好友，交流使用问题与建议。', accountAlt: '暴论哥3.0公众号二维码', follow: '关注公众号', followDetail: '微信扫码，或搜索“暴论哥3.0”。', support: '支持创作', supportDetail: '一份心意，就是继续更新的动力', reward: '微信赞赏', coffee: '觉得好用，可以请作者喝杯咖啡', rewardDetail: '扫码后可自行填写赞赏金额。感谢你支持这款本地 Skill 管理工具继续完善。', rewardAlt: 'Harry 的微信赞赏码'
    }
  },
  settings: {
  ...
  messages: {
    ...
    main: {
      ...
      aiReason: {
        fileTooLarge: '单文件超过 200 KB，不能发送',
        metadataOptional: '宿主 UI 元数据，可选发送',
        textOptional: '文本附件，由你确认后发送',
        scriptNever: '脚本永不发送',
        binaryNever: '二进制文件永不发送',
        overLimit: '超过 200 KB 上限'
      }
    }
  }
} as const;
...
const enUS = {
  common: {
    app: { title: 'Skill Workbench', home: 'Skill Workbench home', author: 'Baolunge 3.0 (same name on WeChat)', authorName: 'Baolunge 3.0' },
  ...
  workbench: {
    ...
    topbar: { searchPlaceholder: 'Search names, descriptions, content, or paths…', searchAria: 'Search Skills', indexReady: 'Index ready', scanRunning: '{{phase}} · {{count}}', manageRoots: 'Manage scan roots', rescanStarted: 'Background scan started', settings: 'Software settings' },
    sidebar: { library: 'Library', all: 'All Skills', editable: 'Editable', duplicates: 'Duplicates', errors: 'Errors', review: 'Needs review', disabled: 'Disabled', trash: 'Trash', disabledHelp: 'Skills inside Claude / WorkBuddy plugins whose enabledPlugins setting is explicitly false', platforms: 'Platforms', platformsHelp: 'Source hosts, not content categories', customHelp: 'A user-added scan directory not recognized as Codex, Claude, or WorkBuddy; no Skill import is needed', hostHelp: 'Skills found in the {{host}} host', categories: 'Categories', categoriesHelp: 'Used only for Workbench filters', more: 'More below', moreAria: 'More content below; continue', history: 'History', settings: 'Software settings', roots: 'Scan folders', privacy: 'Local index · Keys protected by the system' },
    ...
    roots: { title: 'Scan folders', detail: 'Index these folders in place. Adding a project root never migrates or modifies its Skills.', footer: 'Project scans exclude node_modules, .git, session caches, and backups.', add: 'Add project root', added: 'Project root added; a background scan has started', removed: 'Removed scan configuration: {{label}}', discovered: 'Auto-discovered', lastScan: '{{host}} · {{source}} · Last scanned {{date}}', open: 'Open in File Explorer', remove: 'Remove scan configuration', label: { codexShared: 'Codex shared Skills', codexUser: 'Codex user Skills', claudeUser: 'Claude user Skills', workbuddyUser: 'WorkBuddy user Skills', codexPluginCache: 'Codex plugin cache', claudePluginCache: 'Claude plugin cache', claudeMarketplace: 'Claude marketplace candidates', workbuddyPlugins: 'WorkBuddy plugins', trash: 'Workbench Trash' } },
    ...
    guide: { eyebrow: 'First run', title: 'Meet your local Skill map', detail: 'Workbench indexes Skills in place across multiple hosts; it never moves them into its own folder.', welcomeTitle: 'Welcome to Skill Workbench', welcomeDetail: 'Index first, review the result, and confirm every file change yourself.', browse: 'Start browsing', scan: 'Index in place', scanDetail: 'Codex, Claude, and WorkBuddy are detected automatically. Project folders can be added anytime without moving existing Skills.', protect: 'See safety boundaries first', protectDetail: 'Plugin, built-in, marketplace, and cache sources default to read only. Scripts are never run, and deletion moves items to Workbench Trash.', ai: 'AI stays fully on demand', aiDetail: 'Local rules always run first. Only previewed text is sent after you click, and AI suggestions never rewrite files automatically.', manage: 'Manage scan folders', start: 'Get started', note: 'The first full scan runs in the background while cached results appear first. Progress is shown in the top bar.' },
    author: { description: 'Meet the author, add them on WeChat, follow the official account, or support continued development.', footer: 'Contact and support images are provided by the author. Scan them with WeChat.', done: 'Got it', avatarAlt: 'Baolunge 3.0 avatar', label: 'Author', intro: 'Hi, I’m Baolunge 3.0. I built this Workbench to organize, search, and safely manage local Skills. Follow the official WeChat account with the same name for updates and usage notes.', contact: 'Contact and follow', scanWechat: 'Scan with WeChat', wechatAlt: 'QR code to add the author on WeChat', addWechat: 'Add on WeChat', addWechatDetail: 'Add me to discuss questions and suggestions.', accountAlt: 'Baolunge 3.0 official account QR code', follow: 'Follow the official account', followDetail: 'Scan with WeChat or search for “暴论哥3.0”.', support: 'Support the work', supportDetail: 'Your support helps keep updates coming', reward: 'WeChat appreciation', coffee: 'If this is useful, buy the author a coffee', rewardDetail: 'Scan and enter any amount you like. Thank you for supporting this local Skill manager.', rewardAlt: 'Harry’s WeChat appreciation code' }
  },
  settings: {
  ...
  messages: {
    ...
    main: {
      ...
      aiReason: {
        fileTooLarge: 'A single file exceeds 200 KB and cannot be sent',
        metadataOptional: 'Optional host UI metadata',
        textOptional: 'Text attachment; sent only after you confirm',
        scriptNever: 'Scripts are never sent',
        binaryNever: 'Binary files are never sent',
        overLimit: 'Above the 200 KB limit'
      }
    }
  }
};
